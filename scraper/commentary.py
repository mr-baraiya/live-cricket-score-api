...
logger = logging.getLogger("cricket.commentary_scraper")
        ...
        try:
            ...
            for p in soup.find_all(["div", "p"]):
                try:
                    ...
                    logger.warning("Error parsing commentary element: %s", exc)
                    continue

            return CommentaryResponse(status="success", commentary=commentary)

        except Exception as exc:
            logger.error("Scrape commentary failed safely for match %s: %s", match_id, exc)
